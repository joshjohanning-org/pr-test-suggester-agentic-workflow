...
describe('calculateTotal', () => {
  ...
  it('rejects negative tax rates', () => {
    assert.throws(
      () => calculateTotal([{ name: 'Notebook', quantity: 1, unitPrice: 10 }], { taxRate: -0.1 }),
  ...
  it('rejects negative discount amounts', () => {
    assert.throws(
      () => calculateTotal([{ name: 'Notebook', quantity: 1, unitPrice: 10 }], { discountAmount: -5 }),
...
describe('hasItem', () => {
  it('returns true when the item exists in the cart', () => {
    const items = [
  ...
  it('returns false when the item does not exist in the cart', () => {
    const items = [
      { name: 'Notebook', quantity: 1, unitPrice: 4.5 }
    ];
